...
// the camara view need to be reset to get better picture of the rendered
// results
...
class Mandelbulb
{
    ...
    static int iterate(const Vector &v0, double order, unsigned max_cycles)
    {
        ...
        double n = order;
        int i;
        for(i = 0; i < max_cycles && (v.x * v.x + v.y * v.y + v.z * v.z < 2.0);
            i++) {
            double r = do_r(v);
            ...
            Vector vn = {rn * cos_theta * cos_phi, rn * sin_theta * cos_phi,
                         -rn * sin_phi};
            v.x = vn.x + v0.x;
            v.y = vn.y + v0.y;
            v.z = vn.z + v0.z;
  ...
  public:
    Mandelbulb(){};
    Mandelbulb(unsigned width, unsigned height, unsigned depth, double z_offset,
               float range, int blockid, unsigned nblocks = 1)
        : m_width(width), m_height(height), m_depth(depth + 1), m_extents(6),
          m_origin(3), m_data(width * height * (depth + 1)),
          m_z_offset(z_offset), m_range(range), m_nblocks(nblocks),
          m_blockid(blockid)
    {
        m_extents[4] = 0;
    ...
    void SetData(std::vector<char> &stageData)
    {
        ...
        if(byteSize != this->m_data.size() * sizeof(int)) {
            throw std::runtime_error("wrong data length, bytesize " +
                                     std::to_string(byteSize) + " cell size " +
                                     std::to_string(this->m_data.size()));
        }
    ...
    void SetDataVoidPtr(void *rawData, int dataCopySize)
    {
        if(dataCopySize != this->m_data.size() * sizeof(int)) {
            throw std::runtime_error("wrong data length, bytesize " +
                                     std::to_string(dataCopySize) + " cell size " +
                                     std::to_string(this->m_data.size()));
        }
        memcpy(&(this->m_data[0]), rawData, dataCopySize);
    }
    ...
    /*
     ...
     void serialize(A& ar)
     {
       ar(m_width, m_height, m_depth, CEREAL_NVP(m_extents),
     CEREAL_NVP(m_origin), CEREAL_NVP(m_data), m_z_offset, m_range, m_nblocks);
     }
     */
  private:
    size_t m_width;
